...
# Load potentially multiple molecules from the file into a list
ligand_mols_list = Molecule.from_file(mol_in)

# Check if any molecules were loaded
if not ligand_mols_list:
    raise ValueError(f"Could not load any molecules from ligand file: {mol_in}")

# --> FIX: Assume we want the first molecule from the SDF file <--
# If Molecule.from_file returned a list, take the first element.
# If it somehow returned a single Molecule object (less common for SDF), use it directly.
if isinstance(ligand_mols_list, list):
    print(f"Loaded {len(ligand_mols_list)} molecule entries from {mol_in}. Using the first one.")
    ligand_mol = ligand_mols_list[0]
else:
    # This case might occur for file formats guaranteed to have only one molecule
    print(f"Loaded single molecule entry from {mol_in}.")
    ligand_mol = ligand_mols_list
...
system_generator = SystemGenerator(
    ...
    # --> FIX: Pass the single Molecule object inside a list <--
